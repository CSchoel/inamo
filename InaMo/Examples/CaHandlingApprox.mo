...
model CaHandlingApprox "unit test for CaHandling with approximated currents"
  ...
  model DummyCaL
    ...
    SI.Current i = gaussianOff(time, y_min=0, y_max=-3e-10, x0=0.2, sx=200)
                 + gaussianOff(time, y_min=0, y_max=-1e-10, x0=0.23, sx=30)
                 + gaussianOff(time, y_min=0, y_max=-1e-10, x0=-0.1, sx=6);
    inner SI.Current i_ion = i;
  end DummyCaL;
  model DummyNaCa
    extends TransmembraneCaFlow(n_ca=-2);
    SI.Current i = gaussianOff(time, y_min=-0.5e-11, y_max=-4e-11, x0=0.2, sx=200)
                 + gaussianOff(time, y_min=-0.5e-11, y_max=0.3e-11, x0=0.23, sx=50)
                 + gaussianOff(time, y_min=0, y_max=-0.5e-10, x0=-0.1, sx=6);
    inner SI.Current i_ion = i;
  end DummyNaCa;
  DummyCaL cal
