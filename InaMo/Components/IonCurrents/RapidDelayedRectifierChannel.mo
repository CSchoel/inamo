...
model RapidDelayedRectifierChannel "I_K,r"
  ...
  GateTS act_slow(
    redeclare function ftau = gaussianOff(y_min=0.33581, y_max=0.90673+0.33581, x0=-10e-3, sigma=sqrt(988.05/2)/1000),
    redeclare function fsteady = act_steady
  );
  GateTS inact(
    redeclare function ftau = pseudoABTau(
    ...
    redeclare function fsteady = fprod(
      ...
      redeclare function fb = gaussianOff(y_min=1, y_max=-0.3 + 1, sigma=sqrt(500/2)/1000)
    )
  );
