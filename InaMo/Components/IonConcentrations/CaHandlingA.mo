...
model CaHandlingA "Ca handling in Lindblad 1996"
  ...
  ReleaseInact reli;
  ReleaseReact relr;
  DiffSimple nsr_jsr(tau=1) "translocation of Ca2+ between NSR and JSR";
  DiffHL jsr_cyto(vol_dst=v_cyto, p=adjust_to_vmin(1, 1, v_jsr), ka=1, n=2) "release of Ca2+ from JSR into cytosol";
  DiffUptake cyto_nsr(vol_src=v_cyto, vol_dst=v_nsr, i_max=adjust_to_vmin(1, 1, v_nsr)) "uptake of Ca2+ from cytosol into JSR";
