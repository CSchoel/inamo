...
def first_point(path):
    return svgpath.parse_path(path).start


def width(path):
    p = svgpath.parse_path(path)
    return abs(p.point(0)[0] - p.point(1)[0])


def path_by_id(dom, id):
    return dom.xpath("//path[@id = '{}']".format(id))[0].get("d")
...
def reconstruct_full_cells_S7(fname):
    ...
    x0s = [first_point(path_by_id(dom, "x0_"+x))[0] for x in ["am", "an", "n", "nh"]]
